<template>
  <section class="bg-black min-h-content">
    <ul role="list" class="grid h-full grid-cols-1 bg-black sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
      <li v-for="(interview, index) in interviews.interviews" :key="interview.id" class="relative flex items-center justify-center w-full h-full border-b border-r border-white border-opacity-25 fadeIn video" :style="{animationDelay : index/5 + 's'}" @click="getSelectedInterview(interview)">
    ...
    <InterviewVideoModal :is-open="isModalOpen" :interview="selectedInterview" @close="isModalOpen = false" />
  </section>
</template>
...
export default {
  ...
  methods: {
    getSelectedInterview (interview) {
